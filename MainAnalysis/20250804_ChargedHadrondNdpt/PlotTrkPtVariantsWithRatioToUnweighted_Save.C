                                                 ...
                                                 bool logy = false)
{
    ...
    // Set styles
    hUnweighted->SetLineColor(kGray+2);        hUnweighted->SetLineWidth(2); hUnweighted->SetMarkerStyle(20); hUnweighted->SetMarkerColor(kGray+2);
    hNoEvt->SetLineColor(kRed+1);              hNoEvt->SetLineWidth(2);      hNoEvt->SetMarkerStyle(21);      hNoEvt->SetMarkerColor(kRed+1);
    hNoTrk->SetLineColor(kOrange+7);           hNoTrk->SetLineWidth(2);      hNoTrk->SetMarkerStyle(22);      hNoTrk->SetMarkerColor(kOrange+7);
    hNoPartSpecies->SetLineColor(kGreen+2);    hNoPartSpecies->SetLineWidth(2); hNoPartSpecies->SetMarkerStyle(23); hNoPartSpecies->SetMarkerColor(kGreen+2);
    hFull->SetLineColor(kBlue+2);              hFull->SetLineWidth(2);       hFull->SetMarkerStyle(24);       hFull->SetMarkerColor(kBlue+2);

    rNoEvt->SetLineColor(kRed+1);              rNoEvt->SetMarkerStyle(21);      rNoEvt->SetMarkerColor(kRed+1);
    rNoTrk->SetLineColor(kOrange+7);           rNoTrk->SetMarkerStyle(22);      rNoTrk->SetMarkerColor(kOrange+7);
    rNoPartSpecies->SetLineColor(kGreen+2);    rNoPartSpecies->SetMarkerStyle(23); rNoPartSpecies->SetMarkerColor(kGreen+2);
    rFullRatio->SetLineColor(kBlue+2);         rFullRatio->SetMarkerStyle(24);     rFullRatio->SetMarkerColor(kBlue+2);
...
//    pad1->SetGrid();
    ...
    double maxY = std::max({hUnweighted->GetMaximum(), hNoEvt->GetMaximum(),
                            hNoTrk->GetMaximum(), hNoPartSpecies->GetMaximum(), hFull->GetMaximum()});
